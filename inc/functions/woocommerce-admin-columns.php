...
function bbloomer_admin_products_visibility_column_content($column, $product_id)
{
  ...
  if ($column == 'Class Status') {
    ...
    echo get_field('filter_class_status', $product_id);
    if ($cancel_status == 'Cancel') {
      echo 'Cancel';
      //update_post_meta((int)$product_id, "filter_class_status", 'Cancel');
    } else {
      if ($today_2 < $date_2 || $today_2 == $date_2) {
        echo 'Live';
        //update_post_meta((int)$product_id, "filter_class_status", 'Live');
      } else if ($today_2 > $date_2) {
        echo 'Completed';
        //update_post_meta((int)$product_id, "filter_class_status", 'Completed');
      }
    }
  }
