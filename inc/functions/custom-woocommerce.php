...
function exclude_list_per_function($query)
{
  if (is_admin()) {
    ...
    if ($query->is_main_query()) {
      $values = $_GET['filter_class_status'];
      if ($values == 'Completed' || $values == 'Live') {
        $argspost = array(
          'posts_per_page' => -1,
          'post_type' => 'product',
          'meta_query'  => array(
        ...
        $queryproduct = new WP_Query($argspost);
        //echo $queryproduct->found_posts;
        if ($queryproduct->have_posts()) :
          while ($queryproduct->have_posts()) : $queryproduct->the_post();
            $post_id = get_the_ID();
            $get_post_meta = get_post_meta($post_id, 'class_status', true);

            if ($get_post_meta == 'Cancel') {
...
add_action('parse_query', 'exclude_list_per_function');
