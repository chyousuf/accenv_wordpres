...
function wose45466_posts_filter($query)
{
  ...
  if (isset($_GET['post_type']) && !empty($_GET['filter_class_city'])) {
    $type = $_GET['post_type'];
    $text_key_query  = array(
      'relation' => 'AND',
      array(
        'taxonomy' => 'Venues',
        'terms' => $_GET['filter_class_city'],
        'field' => 'ID',
        'operator' => 'IN'
      )
    );
    $query->set('tax_query', $text_key_query);
  }
}
/***************************************************
...
function wose45455_posts_filter($query)
{
  ...
  if (isset($_GET['post_type']) && !empty($_GET['filter_class_status'])) {
    $today_2 = date("Ymd");
    // exit();
    $filter_class_status =  $_GET['filter_class_status'];
    if ($filter_class_status  == 'Live') {
      $query->query_vars['meta_key']     = 'class_date';
      $query->query_vars['meta_value']   = $today_2;
      $query->query_vars['meta_compare'] = '>=';
    } elseif ($filter_class_status == 'Completed') {
      $query->query_vars['meta_key']     = 'class_date';
      $query->query_vars['meta_value']   = $today_2;
      $query->query_vars['meta_compare'] = '<';
    } elseif ($filter_class_status == 'Cancel') {
      $query->query_vars['meta_key']     = 'class_status';
      $query->query_vars['meta_value']   = 'Cancel';
      $query->query_vars['meta_compare'] = '=';
    }
