...
/**************************************************
 * Html tags and Links Remove Form Input
**************************************************/
function custom_text_validation_filter($result, $tag) {  
  $type = $tag['type'];
  $name = $tag['name'];
  if($name == 'r-name' || $name == 'r-company' || $name == 'r-email' || $name == 'r-phone' || $name == 'r-address' || $name == 's-name' || $name == 's-email' || $name == 's-friendname' || $name == 's-friendemail' || $name == 'r-companyname'){
    $value = $_POST[$name];
    $nourl_pattern1 = '(http|https|href)';
    $nourl_pattern = '/<(.+?)[\s]*\/?[\s]*>/si';
    if(preg_match($nourl_pattern,$value) || preg_match($nourl_pattern1,$value)){
...
/**************************************************
 * Html tags and Links Remove Form text Area 
 **************************************************/
function custom_textarea_validation_filter($result, $tag) {  
  ...
  if($name == 'r-description') {
    $value = $_POST[$name];
    $nourl_pattern2 ='(http|https|href)';
    $nourl_pattern3= '/<(.+?)[\s]*\/?[\s]*>/si';
    if(preg_match($nourl_pattern2,$value) || preg_match($nourl_pattern3,$value)){
...
function hide_plugin_trickspanda() {
  ...
  $hidearr = array('xml-sitemap-generator-for-google/xml-sitemap-generator-for-google.php');
...
function mu_hide_plugins_network( $plugins ) {
	$pluginsList = array('xml-sitemap-generator-for-google/xml-sitemap-generator-for-google.php');
	$allPluginList = $plugins;
	foreach ($allPluginList as $key => $val) {
		if (in_array($key, $pluginsList)) {
		  unset( $plugins[$key] );
		}
	}	
    return $plugins;
